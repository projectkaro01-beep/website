...
export const AdminNotices: React.FC = () => {
  ...
  const handlePostNotice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      setMessage({ type: 'success', text: 'Notice broadcasted successfully.' });
      setTitle('');
      setContent('');
      setModalOpen(false);
      await loadNotices();
  ...
  if (loading) {
    return <LoadingSpinner message="Loading institutional notices..." />;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Institutional Bulletins & Notices</h1>
          <p className="text-sm text-slate-500 mt-1">Broadcast urgent updates and announcements to the campus</p>
        </div>
      ...
      {notices.length === 0 ? (
        <EmptyState
          ...
          description="There are currently no announcements in the notice database."
          ...
          {notices.map((n) => (
            <Card key={n.id} className="p-5 flex flex-col justify-between">
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
                <span>By: {n.author?.full_name || 'Administrator'}</span>
                <button
        ...
        <form onSubmit={handlePostNotice} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Campus Holiday / Examination Notice"
